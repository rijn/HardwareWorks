...
void SBN1_Func_Vibration ( uint8_t * _register );
...
void SBN1_USART_Recv ( void )
{
	...
	//  }
	// #else
	while ( UartGetChar ( &g_tUart1, &Recv_Buffer[Recv_Pointer] ) && Recv_Pointer < 1024 )
	{
		Recv_Pointer++;
	};
...
void SBN1_USART_Handle_Reveived ( void )
{
	...
	// DEBUG_PRINT("%d,Buffer=",Recv_Pointer);
	// for(i=0;i<Recv_Pointer;i++)
	// {
	//  DEBUG_PRINT("%02X ", Recv_Buffer[i]);
	// }
	// DEBUG_PRINT("\r\n");
	...
	while ( i < Recv_Pointer && Recv_Pointer > 7 )
	{
		...
		// DEBUG_PRINT("%d,Start Processing, Buffer=",Recv_Pointer);
		// for(temp=0;temp<Recv_Pointer;temp++)
		// {
		//  DEBUG_PRINT("%02X ", Recv_Buffer[temp]);
		// }
		// DEBUG_PRINT("\r\n");
...
void SBN1_USART_Handle_Packet ( uint8_t _payload[], uint8_t _length )
{
	// debug
	// uint8_t i;
	// DEBUG_PRINT("Handle Package\r\n");
	// DEBUG_PRINT("Length = %d [ ", _length);
	// for(i=0;i<_length;i++)
	// {
	//  DEBUG_PRINT("%02X ", _payload[i]);
	// }
	// DEBUG_PRINT("]\r\n");
	// debug
	...
	switch ( _payload[0x00] )
	{
		...
		case SBN1_OP_READ_REQUEST:
		{
			...
			switch ( _mode )
			{
				...
				case 0x01:
				{
					...
					#ifdef SBN1_DEBUG
					DEBUG_PRINT ( "  Single read from %02X\r\n", _address );
					#endif
					SBN1_Func_Read ( temp );
					break;
				}
				...
				{
					#ifdef SBN1_DEBUG
					DEBUG_PRINT ( "  Start contineous read from %02X\r\n", _address );
					#endif
				...
				{
					#ifdef SBN1_DEBUG
					DEBUG_PRINT ( "  Stop read from %02X\r\n", _address );
					#endif
		...
		case SBN1_OP_VIBRATION_REQUEST:
		{
			// DEBUG_PRINT ( "received vibration request\n" );
			SBN1_Func_Vibration ( _payload );
			// uint8_t temp[2];
			// _address = _payload[0x01];
...
void SBN1_Func_Vibration ( uint8_t * _register )
{
	uint8_t * address = & _register[0x01], i;

	nrf24l01ChangeAddress ( ( *address ) );

	for ( i = 0; i < 13; i++ )
	{
		nRF_SendBuffer[i] = _register[i];
	}
	...
	if ( i == TX_DS )
	{
		...
		// DEBUG_PRINT("RX_DR = %02X\r\n", i);
		if ( i == RX_DR && nRF_ReceiveBuffer[0x00] == SBN1_OP_VIBRATION_RESPONSE )
		{
			ledSet ( 0, 1 );
			SBN1_USART_Send_Packet ( nRF_ReceiveBuffer, 0x04 );
			ledSet ( 0, 0 );
			ledSet ( 1, 0 );
			return;
		}
	...
	// if failed
	SBN1_Clear_Buffer ( Temp_Buffer, SBN1_BUFSIZ );
	Temp_Buffer[0x00] = SBN1_OP_VIBRATION_RESPONSE;
	Temp_Buffer[0x01] = * address;
	Temp_Buffer[0x02] = 0x00;
	Temp_Buffer[0x03] = 0x00;
	SBN1_USART_Send_Packet ( Temp_Buffer, 0x04 );
}
...
void SBN1_Func_SelfCheck ( uint8_t * _register )
{
	...
	#ifdef SBN1_DEBUG
	DEBUG_PRINT ( "  SelfCheck %02X\r\n", *address );

	DEBUG_PRINT ( "Send [ " );
	Temp_Buffer[0x00] = *address;
	Temp_Buffer[0x01] = 0x01;
	SBN1_USART_Send_Packet ( Temp_Buffer, 0x02 );
	DEBUG_PRINT ( "]\r\n" );
	#endif
	...
	// DEBUG_PRINT("  SelfCheck %02X - ", (*address));
...
void sbn1Test(void)
{
	// uint8_t _payload[1] = {0x01};
	DEBUG_PRINT("start test\n");
	SBN1_Create_Task ( 0x0101, SBN1_Func_Search, 0x01, 0x01, 0x01 );
	DEBUG_PRINT("start searching\n");
}
