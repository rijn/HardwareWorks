...
static inline int reg_int_cb(struct int_param_s *int_param)
{
    //return msp430_reg_int_cb(int_param->cb, int_param->pin, int_param->lp_exit,
   //     int_param->active_low);
	 //Implement stm32 EXTI ISR callback
	return 0;
}
 ...
 *  @return     0 if successful.
 */
int mpu_init(struct int_param_s *int_param, uint8_t addr)
{
    unsigned char data[6], rev, result = 0;

    DEBUG_PRINT("mpu_init => update device addr from %02X to %02X\n", st.hw->addr, addr);

    st.hw->addr = addr << 1;

    DEBUG_PRINT("mpu_init => Reset Device %02X\r\n", addr);
    /* Reset device. */
    data[0] = BIT_RESET;
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    // DEBUG_PRINT("mpu_init => read mem rev\r\n");
    // if (mpu_read_mem(MPU6500_MEM_REV_ADDR, 1, &rev))
    //     return -1;
    // if (rev == 0x1)
    //     st.chip_cfg.accel_half = 0;
    // else {
    //     DEBUG_PRINT("Unsupported software product rev %d.\n", rev);
    //     // log_e("Unsupported software product rev %d.\n", rev);
    //     return -1;
    // }
    ...
    DEBUG_PRINT("mpu_init => write fifo\r\n");
...
#ifdef AK89xx_SECONDARY
    // DEBUG_PRINT("setting up_compass\r\n");
    result = setup_compass();
    DEBUG_PRINT("setup_compass => %d\r\n", result);
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_fsr(unsigned short *fsr)
{
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    DEBUG_PRINT(" mpu_set_sensors => Success\r\n");
    return 0;
}
...
#define LOAD_CHUNK  (16)
    unsigned char cur[LOAD_CHUNK]={0}; unsigned char tmp[2];
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        if (mpu_read_mem(ii, this_write, cur))
            return -1;
				int te = memcmp(firmware+ii, cur, this_write);
				if(te)
...
int setup_compass(void)
{
    ...
    for (akm_addr = (0x0C)<<1; akm_addr <= (0x0F)<<1; akm_addr++) {
        ...
        // DEBUG_PRINT("akm_addr [%02X], data [%02X] \r\n", akm_addr, data);
    ...
    /*
     * Original code from Github is incorrect
     *
     * Shifting compass_addr should be done after configuring AK compass
     */
