 ...
 * 
 * @return Charater read from Rx queue. 0 if Rx buffer is empty.
 */
static char USART_getchar() {
    uint8_t data = 0;
    ...
    if (!QUEUE.dequeue(&Rxq, &data)) {
        // This should not happen because we shoule check Rx buffer is not
        // empty before calling this function by USART.getchar().
    }
    enableIrq();
    return data;
}
...
static void dmaUsartEcho() {
    ...
    char chs[size];
    uint16_t count = 0;
    while (!QUEUE.isEmpty(USARTBuf.Rxq)) {
        if (count >= size) {
            dmaUsartSend(chs, size);
            count = 0;
        }
        char c = USART.getchar();
        chs[count++] = c;
    }
    if (count > 0) {
        dmaUsartSend(chs, count);
    }
}
